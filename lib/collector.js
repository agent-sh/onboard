...
function getRepoIntel(cwd) {
  try {
    ...
    const projectInfo = safeQuery(binary, ['repo-intel', 'query', 'project-info', '--map-file', mapFile, cwd]);
...
function extractRepoMapSummary(data, format) {
  if (format === 'symbols') {
    // repo-intel.json symbols format
    const files = Object.keys(data);
    const totalSymbols = Object.values(data).reduce((sum, f) =>
      sum + (f.definitions ? f.definitions.length : 0), 0);
    const exportSummary = {};
    for (const [filePath, syms] of Object.entries(data)) {
      if (syms.exports && syms.exports.length > 3) {
        exportSummary[filePath] = syms.exports.map(s => s.name);
      }
    }
