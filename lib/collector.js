...
function getRepoIntel(cwd) {
  try {
    ...
    try {
      const { binary: b, libRoot } = require('./agentsys').get();
      binary = b;
      getStateDirPath = require(`${libRoot}/platform/state-dir`).getStateDirPath;
    } catch { return null; }
    ...
    // Entry-points is THE newcomer answer to "where does this thing
    // start?" — Cargo [[bin]] targets, main() functions, package.json
    // bin scripts, framework-loaded configs. Authoritative (per-file
    // AST + manifest parse) so we prefer it over inferring from
    // manifest scripts.
    const entryPointsRaw = safeQuery(binary, ['repo-intel', 'query', 'entry-points', '--map-file', mapFile, cwd]);
    const entryPoints = Array.isArray(entryPointsRaw) ? entryPointsRaw : (entryPointsRaw?.entryPoints || []);

    // Slop-fixes: partition into the subsets that help a newcomer
    // orient. Orphan-exports and passthrough-wrappers flag parts of
    // the codebase that are either dead or trivially delegating —
    // useful for "which areas should I skip on day one" guidance.
    // Always-true-condition and commented-out-code indicate neglect.
    // Single-pass partition + count so we walk the fixes once and
    // each category string lives in exactly one place.
    const slopRaw = safeQuery(binary, ['repo-intel', 'query', 'slop-fixes', '--map-file', mapFile, cwd]);
    ...
    for (const fix of slopFixes) {
      const key = SLOP_CATEGORIES[fix.category];
      if (!key) continue;
      slop.counts[key] += 1;
      if (slop[key].length < SAMPLE_CAP) slop[key].push(fix);
    }

    return { onboard, hotspots, conventions, projectInfo, entryPoints, slop };
  } catch {
    return null;
  }
}
...
function getRepoMap(cwd) {
  try {
    let stateDirPath;
    try {
      const { libRoot } = require('./agentsys').get();
      stateDirPath = require(`${libRoot}/platform/state-dir`).getStateDirPath(cwd);
    } catch {
...
function extractRepoMapSummary(data, format) {
  ...
  // repo-map.json files format. agentsys writes
  // files[path].symbols = { exports, functions, classes, types, constants };
  // older maps used a flat array of { name, exported }.
  const files = Object.keys(data);
  const totalSymbols = Object.values(data).reduce((sum, f) => sum + symbolCount(f && f.symbols), 0);
  const exportEntries = Object.entries(data)
    ...
    .slice(0, MAX_KEY_EXPORT_FILES);
  const exportSummary = {};
  for (const [filePath, names] of exportEntries) {
    exportSummary[filePath] = names.slice(0, 15);
  }
  return { totalFiles: files.length, totalSymbols, keyExports: exportSummary };
}
