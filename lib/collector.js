...
function getRepoIntel(cwd) {
  try {
    ...
    // Cap each subset so the agent prompt stays bounded.
    ...
    const byCategory = (cat) => slopFixes.filter((f) => f.category === cat).slice(0, 10);
    const slop = {
      orphanExports: byCategory('orphan-export'),
      passthroughWrappers: byCategory('passthrough-wrapper'),
      alwaysTrueConditions: byCategory('always-true-condition'),
      commentedOutCode: byCategory('commented-out-code'),
      counts: {
